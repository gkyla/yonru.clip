<template>
  <div
    v-if="state"
    class="bg-transparent flex flex-col transition-all duration-500"
  >
    <!-- Diagnostic fallback if audit is missing -->
    <div v-if="!audit" class="p-8 text-center bg-surface-card/30">
      <Icon
        name="ri:loader-4-line"
        class="text-3xl text-accent-500 animate-spin mb-2"
      />
      <p class="text-accent-500 text-xs font-black uppercase tracking-widest">
    ...
    <template v-else>
      <div
        class="px-3 h-10 border-b border-surface-border/30 flex items-center justify-between cursor-pointer hover:bg-white/[0.03] shrink-0 transition-all duration-500"
        :class="
          audit && audit.flaggedWords.length > 0
      ...
      >
        <div class="flex items-center gap-2">
          ...
          <span
            ...
            class="relative flex h-1.5 w-1.5 ml-0.5"
          >
            <span
        ...
        <div class="flex items-center">
          ...
          <Icon
            ...
            class="text-slate-500 text-lg ml-1"
          />
        </div>
      </div>
      ...
      >
        <div class="overflow-hidden">
          <!-- Safety Bento Card -->
          <div
            class="p-4 space-y-4 custom-scrollbar min-h-0 overflow-y-auto"
          ...
          >
            ...
            <div
              class="bg-white/[0.02] border border-white/5 rounded-2xl p-4 flex justify-center items-center gap-4 relative group hover:border-white/10 transition-colors"
            >
              ...
              <div
                class="relative w-14 h-14 flex items-center justify-center flex-shrink-0 z-10"
              >
                <svg
              ...
              <div class="flex flex-col gap-1 z-10 flex-1 min-w-0">
                <span
                  class="text-[9px] text-slate-500 font-black uppercase tracking-widest leading-none"
                  >Safety Eligibility</span
                >
                <span
                  class="text-xs font-bold tracking-tight uppercase truncate"
                  :class="scoreTextClass"
                >
                  {{ scoreLabel }}
                </span>
              ...
              <!-- Info Tooltip (Right) -->
              <div
                class="relative group/tooltip flex items-center flex-shrink-0 z-[99]"
              >
                <Icon
                  name="ri:question-line"
                  class="text-slate-500 hover:text-slate-300 text-sm cursor-help transition-colors"
                />
                <div
                  class="absolute top-full right-0 mt-1.5 w-60 p-2.5 bg-surface-dark/95 border border-surface-border/80 rounded-xl shadow-black/80 shadow-[0_12px_40px_rgba(0,0,0,0.95)] text-[9px] text-slate-300 leading-normal opacity-0 group-hover/tooltip:opacity-100 pointer-events-none transition-opacity duration-200 z-30 tracking-normal normal-case"
                >
                  <p
                    class="font-bold text-slate-200 mb-1 border-b border-white/5 pb-1"
                  >
                  ...
                  <ul class="space-y-1">
                    <li>
                      <span class="text-emerald-400 font-semibold"
                        >Excellent (&ge;90):</span
                      >
                      No sensitive keywords; safe zone & readability optimal for
                      maximum reach.
                    </li>
                    <li>
                      <span class="text-accent-400 font-semibold"
                        >Good (&ge;70):</span
                      >
                      Minor visual risk (e.g. slight safe zone overlap or mild
                      slang detected).
                    </li>
                    <li>
                      <span class="text-amber-400 font-semibold"
                        >Caution (&ge;40):</span
                      >
                      Flagged shadowban keywords detected; distribution may be
                      restricted.
                    </li>
                    <li>
                      <span class="text-rose-400 font-semibold"
                        >High Risk (&lt;40):</span
                      >
                      Critical policy violations or profanity; severe risk of
                      shadowban.
                    </li>
                  </ul>
                </div>
            ...
            <div class="grid grid-cols-1 gap-3">
              ...
              <div
                ...
                :class="
                  state.layoutAudit.value.isSafe
                ...
                <div class="flex items-start gap-3">
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs text-slate-200 font-medium">
                      {{ state.layoutAudit.value.reason }}
                    </p>
                  </div>
                </div>
              ...
              <!-- Safety: Subtitles -->
              <div
                class="bg-surface-card/50 border border-surface-border rounded-xl p-3 flex flex-col gap-3"
              >
                <div class="flex items-start gap-3">
                  <div
                    class="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0"
              ...
              <div
                class="border rounded-xl p-3 flex flex-col gap-2.5 transition-all duration-500"
                :class="
                  state.readabilityAudit.value.isSafe
                    ? 'bg-surface-card/50 border-surface-border'
                    : 'bg-amber-500/5 border-amber-500/20'
                "
              >
                <div class="flex items-start gap-3">
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs text-slate-200 font-medium">
                      {{ state.readabilityAudit.value.reason }}
                    </p>
                  </div>
                </div>
            ...
            <div class="pt-2 flex flex-col gap-2">
              <button
                v-if="
                  !state.isWarningIgnored.value && audit && audit.score < 100
                "
              ...
              >
                <Icon
                  ...
                  class="text-xs group-hover:rotate-90 transition-transform"
                />
    ...
    <Teleport to="body">
      ...
      >
        ...
        >
          ...
          <!-- Modal Card -->
          <div
            class="relative bg-[#0e0e12] border border-rose-500/30 rounded-2xl p-6 max-w-md w-full shadow-2xl space-y-4 text-white z-10"
          >
            <!-- Header Icon & Title -->
            <div class="flex items-start gap-3">
              <div
                class="w-10 h-10 rounded-xl bg-rose-500/10 border border-rose-500/20 flex items-center justify-center flex-shrink-0 text-rose-500"
              >
                <Icon name="ri:alert-line" class="text-xl" />
              </div>
              <div class="flex-1">
                <h4
                  class="text-sm font-black uppercase tracking-wider text-rose-400"
                >
                  Ignore Safety Warnings?
                </h4>
                <p class="text-xs text-slate-400 mt-1 leading-relaxed">
                  Ignoring safety warnings will bypass shadowban keyword
                  censoring and safe zone alignment checks for this video clip.
                </p>
              </div>
            </div>
            ...
            <div
              class="bg-rose-500/5 border border-rose-500/15 rounded-xl p-3 text-[11px] text-slate-300 space-y-1.5 leading-normal"
            >
              ...
              <ul
                class="list-disc list-inside text-slate-400 space-y-0.5 pl-1 text-[10px]"
              >
                <li>
                  Subtitles may collide with platform UI controls (TikTok,
                  Reels, Shorts).
                </li>
                <li>
                  Sensitive words may be flagged by automated shadowban
                  algorithms.
                </li>
              </ul>
            </div>
