      ...
      <div class="flex flex-col h-full w-full overflow-visible">
        ...
        <div class="flex-1 min-h-0 relative overflow-visible flex flex-col">
          ...
          <Transition
            ...
            enter-from-class="opacity-0"
            enter-to-class="opacity-100"
            leave-active-class="transition-opacity duration-75 ease-in absolute left-0 top-0 bottom-0 w-[280px] pointer-events-none"
            leave-from-class="opacity-100"
            leave-to-class="opacity-0"
          >
            <div v-if="!isCollapsed" class="h-full w-full flex flex-col min-h-0">
              ...
              <div class="p-3 pb-0 shrink-0 relative overflow-visible z-50">
                <div class="relative profile-popover-container" ref="profileContainerRef">
                  ...
                  >
                    <div class="flex items-center gap-2.5 min-w-0">
                      <div class="w-8 h-8 rounded-full bg-accent-500/10 border-2 border-accent-500/25 flex items-center justify-center text-accent-500 font-bold text-xs shrink-0 relative">
                        <span>{{ activeProfile.avatarInitial }}</span>
                        <span class="absolute -bottom-0.5 -right-0.5 w-2 h-2 rounded-full bg-emerald-500 border border-[#09090b]"></span>
                      </div>
                      <div class="overflow-hidden min-w-0">
                        <div class="text-[11px] font-semibold text-white truncate leading-tight flex items-center gap-1.5">
                          <span>{{ activeProfile.name }}</span>
                        </div>
                        <p class="text-[9px] text-white/60 truncate mt-0.5">{{ activeProfile.niche }}</p>
                      </div>
                    </div>
                    <Icon 
                  ...
                  >
                    <div 
                      ...
                      @click.stop
                      class="absolute left-full top-0 ml-3 w-64 bg-[#121216] border border-white/10 rounded-2xl shadow-2xl p-2.5 z-[9999] flex flex-col gap-1 backdrop-blur-xl"
                    >
                      <div class="px-2 py-1.5 text-[9px] font-bold uppercase tracking-wider text-white/60 border-b border-white/[0.06] flex items-center justify-between">
                        <span>Switch Niche Profile</span>
                        <span class="text-[8px] text-accent-500 font-mono">{{ profiles.length }} PROFILES</span>
                      </div>

                      <!-- Profile Items List -->
                      <div class="flex flex-col gap-1 max-h-48 overflow-y-auto custom-scrollbar my-1">
                        <button 
                          v-for="profile in profiles" 
                          :key="profile.id"
                          @click="selectProfile(profile)"
                          class="flex items-center justify-between p-2 rounded-lg transition-all text-left cursor-pointer group"
                          :class="activeProfile.id === profile.id ? 'bg-accent-500/10 text-accent-500 font-bold border border-accent-500/20' : 'hover:bg-white/[0.04] text-white/85 border border-transparent'"
                        >
                          <div class="flex items-center gap-2.5 min-w-0">
                            <div 
                              class="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 border"
                              :class="activeProfile.id === profile.id ? 'bg-accent-500/20 text-accent-500 border-accent-500/30' : 'bg-white/[0.04] text-white/70 border-white/[0.06]'"
                            >
                              {{ profile.avatarInitial }}
                            </div>
                            <div class="overflow-hidden min-w-0">
                              <p class="text-[11px] font-medium truncate leading-tight">{{ profile.name }}</p>
                              <p class="text-[9px] text-white/60 truncate">{{ profile.niche }}</p>
                            </div>
                          </div>
                          <Icon v-if="activeProfile.id === profile.id" name="lucide:check" class="text-xs text-accent-500 shrink-0 ml-2" />
                        </button>
                      </div>

                      <!-- Add New Niche Profile Action -->
                      <div class="pt-1 border-t border-white/[0.06] flex flex-col gap-1">
                        <button 
                          @click="openAddProfileModal"
                          class="flex items-center gap-2 w-full px-2.5 py-1.5 rounded-lg text-white/75 hover:text-white hover:bg-white/[0.04] text-[11px] transition-colors cursor-pointer"
                        >
                          <Icon name="lucide:plus" class="text-xs" />
                          <span>Add Niche Profile</span>
                        </button>
                      </div>
                    </div>
                  </Transition>
          ...
          <Transition
            ...
            enter-from-class="opacity-0"
            enter-to-class="opacity-100"
            leave-active-class="transition-opacity duration-75 ease-in absolute left-0 top-0 bottom-0 w-16 pointer-events-none"
            leave-from-class="opacity-100"
            leave-to-class="opacity-0"
          >
          ...
          <div class="flex flex-col items-center gap-3.5 w-full px-2">
            <!-- Collapsed Profile Avatar Trigger (Circular Avatar) -->
            <div class="relative profile-popover-container">
              <button 
                @click.stop="toggleProfileMenu"
                class="w-10 h-10 rounded-full bg-accent-500/10 border border-accent-500/25 flex items-center justify-center text-accent-500 font-bold text-xs cursor-pointer relative hover:border-accent-500/50 transition-all"
                title="Switch Niche Profile"
              >
                <span>{{ activeProfile.avatarInitial }}</span>
                <span class="absolute -bottom-0.5 -right-0.5 w-2 h-2 rounded-full bg-emerald-500 border border-[#09090b]"></span>
              </button>
              
              <!-- Floating Niche Profile Popover Menu in Collapsed Mode (Flyout to the RIGHT) -->
              <Transition
                enter-active-class="transition duration-150 ease-out"
                enter-from-class="opacity-0 translate-x-2 scale-95"
                enter-to-class="opacity-100 translate-x-0 scale-100"
        ...
        <div class="mt-auto border-t border-white/[0.08] bg-black/20 shrink-0 relative overflow-visible p-3 min-h-[110px] flex flex-col justify-center">
          ...
          <Transition
            ...
            enter-from-class="opacity-0"
            enter-to-class="opacity-100"
            leave-active-class="transition-opacity duration-75 ease-in absolute left-0 top-0 bottom-0 w-[280px] p-3 pointer-events-none"
            leave-from-class="opacity-100"
            leave-to-class="opacity-0"
          >
            <div v-if="!isCollapsed" class="flex flex-col gap-2.5 w-full">
          ...
          <Transition
            ...
            enter-from-class="opacity-0"
            enter-to-class="opacity-100"
            leave-active-class="transition-opacity duration-75 ease-in absolute left-0 top-0 bottom-0 w-16 p-2 pointer-events-none"
            leave-from-class="opacity-100"
            leave-to-class="opacity-0"
          >
...
const profiles = ref<NicheProfile[]>([
  { id: 'default', name: 'Personal Shorts', niche: 'General Niche', avatarInitial: 'P' },
  { id: 'finance', name: 'FinTrack Shorts', niche: 'Finance & Tech', avatarInitial: 'F' },
  { id: 'gaming', name: 'Gaming Highlights', niche: 'Gaming & Streams', avatarInitial: 'G' }
])
...
const activeProfile = computed<NicheProfile>(() => {
  return profiles.value.find(p => p.id === activeProfileId.value) || profiles.value[0] || defaultFallbackProfile
})
...
function openAddProfileModal() {
  isProfileMenuOpen.value = false
  const name = window.prompt?.('Enter new Niche Profile Name (e.g., Podcast Shorts):')
  if (name && name.trim()) {
    const niche = window.prompt?.('Enter Niche Category (e.g., Podcasts / Talk):') || 'Custom Niche'
    ...
    if (import.meta.client) {
      ...
      localStorage.setItem('yonru_active_niche_profile', newProfile.id)
    }
  }
}
